...
| IV Compare | two registered MPR viewers, matched cross-section | ✅ backend `/compare-slice-map` (cached registration, **gated on inlier ≥0.30**) + UI both frontends (React CompareView reference/target + reliability banner; trame compare mode byte-identical, register-once). Verified: React live inlier 0.925→reliable banner; trame QA live 0.144→unreliable (never hidden) |
| V AR MVP | valid GLB opens in native mobile AR | ✅ `core/export/mesh.py` GLB (colour bake, 250k tri cap) + `GET /model.glb` (real session → 1.4 MB valid glTF) + React `<model-viewer>` (ar-modes webxr/scene-viewer/quick-look; Android GLB, iOS USDZ-absent stated) + trame GLB-for-AR download. 4 GLB tests |
| VI AR/WebXR | in-AR clipping-plane cross-section (device-gated) | ✅ React `ar/ClipViewer.jsx` (three.js): loads /model.glb, clips X/Y/Z with position slider, `navigator.xr.isSessionSupported('immersive-ar')` feature-detect → "Enter AR" only when supported else graceful note; 3D clip works everywhere. Verified: real Z-cut render + fallback headless |
| VII Oblique / any cross-section | 3D cut ↔ 2D reformat matched at *every* pixel; arbitrary plane | ✅ backend `core/viz/slice.oblique_slice` (any plane, exact pixel↔world inverse) + `POST /oblique-slice` (5 tests) + UI both frontends: React ObliqueView (azimuth/elevation/position tiltable plane, translucent plane actor in 3D, exact 2D-click→3D marker via `obliquePixelToWorld`) and trame plane widget (`qa_trame_oblique.py`: tilted plane, panel byte-identical to `render_oblique_png`, exact pixel↔world roundtrip incl. corner, display-only). Verified real tilted-plane screenshot |

**User acceptance for VI/VII (explicit) — MET:** the 3D bone and the 2D images are matched at *all* points (exact pixel↔world inverse; a click on the 2D reformat lands on the precise 3D point, and the 3D cut plane fully defines the 2D image), and the user can view *any orientation* of cross-section (freely tiltable azimuth/elevation plane in React; interactive plane widget in trame). Shipped + verified in both frontends.

## Imaging phases — ALL COMPLETE
Phases I–VII done and verified: design → slice backend → MPR viewer → compare (quality-gated) → AR (GLB/model-viewer) → WebXR clip → oblique/arbitrary cross-section, at parity across both frontends. Every gate above is ✅ with its named verification executed green. Honesty rails intact throughout: array-oriented (never radiological A/P/S/I), research/de-identified/not-for-diagnosis, registration honestly gated, nothing fabricated. Automated watchdog: GREEN (registry, config, git-PHI, de-identify, parity, pytest).
