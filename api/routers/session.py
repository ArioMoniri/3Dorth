...
# Local override: point the demo at ANY volume/zip/DICOM-dir on this machine via the
# THREEDORTH_DEMO env var (and THREEDORTH_DEMO_FOLLOWUP for a 2nd series). Used to
# demo your OWN scans locally WITHOUT committing them — the served session meta only
# carries the DICOM SeriesDescription + a PatientID *hash*, never a patient name, and
# the raw files stay .gitignored. Absent env var => the shipped de-identified demo.
_ENV_DEMO = os.environ.get("THREEDORTH_DEMO", "").strip()
_ENV_DEMO_FOLLOWUP = os.environ.get("THREEDORTH_DEMO_FOLLOWUP", "").strip()
_DEMO = (
    Path(_ENV_DEMO).expanduser() if _ENV_DEMO
    else _DEIDENTIFIED_DEMO if _DEIDENTIFIED_DEMO.exists()
    else _PATIENT_DEMO
)
# Optional 2nd series. An explicit env follow-up wins. If a local baseline override
# is set with NO follow-up, stay single-series (never mix the user's own scan with
# the shipped SYNTHETIC follow-up). Otherwise use the DE-IDENTIFIED synthetic
# follow-up (scripts/make_multi_demo.py) so the multi-series Mode B demo works out
# of the box. Absent => single-series demo (backward compatible).
if _ENV_DEMO_FOLLOWUP:
    _DEMO_FOLLOWUP = Path(_ENV_DEMO_FOLLOWUP).expanduser()
elif _ENV_DEMO:
    _DEMO_FOLLOWUP = Path("/nonexistent/3dorth-no-followup")  # .exists() -> False
else:
...
class FiguresReq(BaseModel):
    ...
    target_side: str | None = None
    region_label: int | None = None
    whole_bone: bool = False   # must match /analyze's cache key to reuse its result
    params: dict = {}
...
# ----------------------------------------------------------------------------
# Multi-series sessions
# ----------------------------------------------------------------------------
# A session can hold MULTIPLE uploaded SERIES (e.g. baseline + follow-up visits),
# each possibly bilateral. The FIRST series keeps plain side names ("left"/"right"/
# "mesh") for full backward compatibility; further series are namespaced by id,
# e.g. "s1/left", "s1/right". Mode-B compares any two side keys — including two
# series' matching sides ("left" vs "s1/left" = baseline-left vs follow-up-left) —
# with no change to compare_sides (it just receives the two resolved side dicts).
...
def _new_session(arr, spacing, meta, layout: str = "auto") -> dict:
    ...
    SESSIONS[sid] = {"arr": arr, "spacing": spacing, "meta": meta, "sides": sides,
                     "series": series, "series_meta": {"s0": meta}, "layout": layout}
    return {"session_id": sid, "meta": meta, "sides": list(sides.keys()),
            "series": series, "all_sides": list(sides.keys()),
...
def _new_mesh_session(mesh, meta) -> dict:
    ...
    series = [_series_entry("s0", meta, sides.keys())]
    SESSIONS[sid] = {"arr": None, "spacing": None, "meta": meta, "sides": sides,
                     "series": series, "series_meta": {"s0": meta}, "is_mesh": True}
    return {"session_id": sid, "meta": meta, "sides": ["mesh"],
            "series": series, "all_sides": ["mesh"],
            "is_bilateral": False, "is_mesh": True}


def _add_series(s: dict, session_id: str, arr, spacing, meta, mesh=None) -> dict:
    """Append a new series to an existing session; its sides are namespaced by id
    ("s1/left", …). Returns the upload response shape."""
    series_id = f"s{len(s.get('series', []))}"
    if mesh is not None:
    ...
    s["sides"].update(new_sides)
    s.setdefault("series_meta", {})[series_id] = meta
    entry = _series_entry(series_id, meta, new_sides.keys())
    s.setdefault("series", []).append(entry)
    return {"session_id": session_id, "series_id": series_id, "added": True,
...
def create_session(layout: str = "bilateral") -> dict:
    ...
    # Keep the REAL SeriesDescription when a local env override is used (so you see
    # e.g. "CORONAL"); only relabel the shipped synthetic demo as "baseline (demo)".
    if _DEMO_FOLLOWUP.exists() and not _ENV_DEMO:
        meta["series"] = "baseline (demo)"  # clarity when a 2nd series is present
    resp = _new_session(arr, spacing, meta, layout=layout)

    # Auto-load the follow-up as a 2nd series (baseline vs follow-up) so the
    # multi-series Mode B demo works out of the box. Best-effort: any failure
    # leaves the single-series demo intact.
    if _DEMO_FOLLOWUP.exists():
        try:
            sid = resp["session_id"]
            arr2, spacing2, meta2 = pipeline.load_volume_from_source(_DEMO_FOLLOWUP, WORKDIR)
            if not _ENV_DEMO_FOLLOWUP:
                meta2["series"] = "follow-up (demo)"
            add = _add_series(_get_session(sid), sid, arr2, spacing2, meta2)
            resp["series"] = add["series"]
            resp["all_sides"] = add["all_sides"]
            resp["sides"] = add["all_sides"]
        except Exception:  # noqa: BLE001 — demo must still load single-series
            pass
    return resp
...
async def upload(
    ...
    """Ingest a scan or mesh. With no ``session_id`` a NEW session is created (one
    series, s0, plain side names — backward compatible). With ``session_id`` the
    file is ADDED to that session as another series (namespaced sides) so it can be
    compared against the existing series (e.g. baseline vs follow-up)."""
    ...
    add_to = _get_session(session_id) if session_id else None
    ...
    if is_mesh(dest):
        ...
        if add_to is not None:
            return _add_series(add_to, session_id, None, None, meta, mesh=mesh)
    ...
    meta.setdefault("series", name)
    if add_to is not None:
        return _add_series(add_to, session_id, arr, spacing, meta)
...
def _result_for_figures(s: dict, req: "FiguresReq", params) -> tuple[object, str, list[dict] | None]:
    ...
    # Key MUST match /analyze's exactly (side|region|whole_bone|params) so the
    # figures endpoint reuses the interactive result instead of recomputing.
    key = hashlib.sha256(
        f"{side_name}|{req.region_label}|{req.whole_bone}|{json.dumps(req.params, sort_keys=True)}".encode()
    ).hexdigest()[:16]
    cache = s.setdefault("analyze_cache", OrderedDict())
    hit = cache.get(key)
    if hit is not None:
