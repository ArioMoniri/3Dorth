...
if [ -z "$PICK" ]; then
  ...
  err "  1) PUBLIC LINK via your laptop (zero infra) — run ON YOUR LAPTOP, not here:"
  err "       ./scripts/share_from_laptop.sh    (relays the pod through your SSH + opens a Cloudflare link from the laptop)"
  err "  2) Just for you (no outsiders) — one SSH hop from your machine:"
  err "       ssh -L ${APP_PORT}:127.0.0.1:${APP_PORT} -p 30405 root@10.6.110.10   then open http://localhost:${APP_PORT}"
  err "  3) Reverse-forward to a relay YOU control on 443:"
  err "       SSH_RELAY=user@your-host:443 ./scripts/tunnel.sh ${APP_PORT}   (relay needs GatewayPorts clientspecified)"
  err "  4) Expose a cluster Ingress / NodePort via your admin."
